...
def init_vote_session_list(mysql):
    ...
    for vote_session_key in vote_session_list:
        ...
        result = ExecuteQuerySelect('otp', {'columns2select':'*', 'vote_id':vote_session_key}, mysql)#, 'status':'available'}, mysql)
        for row in result:
        ...
        result = ExecuteQuerySelect('secret', {'columns2select':'*', 'vote_id':vote_session_key}, mysql)#, 'status':'enabled'}, mysql)
        for row in result:
